...
class ControllerInformationNews extends Controller {
	...
	public function index() {
		...
		foreach ($all_news as $news) {
			...
			if (!empty($image)){
				...
				$cover .= '<a href="'.$this->url->link('information/news/read', 'news_id=' . $news['news_id']).'" title="'.$news['title'].'">';
			    $cover .= $image[0];
		        $cover .= '</a>';
				$cover .= '</div>';
   ...
   public function read() {
      ...
      if ($news) {
         $this->data['breadcrumbs'][] = array(
            'text' => $news['title'],
            'href' => $this->url->link('information/news/read', 'news_id=' . $news_id),
            'separator' => $this->language->get('text_separator')
         );
 
        $this->document->setTitle($news['title']);
		$this->document->setDescription(self::_clear_whitespace(html_entity_decode(self::limit_words($news['description'],84,''))));
		$this->document->setKeywords(self::_explode_keywords(html_entity_decode($news['description'])));
		$this->document->addLink($this->url->link('information/news/read', 'news_id=' . $this->request->get['news_id']), 'canonical');
         ...
         if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/news_detail.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/information/news_detail.tpl';
         } else {
            $this->template = 'default/template/information/news_detail.tpl';
         }
   ...
   public function _explode_keywords($string = '', $delimiter = ', ') {
		if (empty($string))
			return;
		...
		$return = self::_clear_whitespace($return);
		
		if (!empty($return))
			return trim(self::limit_words($return,64,''),"\x00..\x1F");
	}
	
	public function _clear_whitespace($string = '') {
		$return =  trim(strip_tags($string),"\x00..\x1F");
		$return = str_replace(array('"','\''),'', $return);
